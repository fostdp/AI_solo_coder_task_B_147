...
func NewMaintenanceManager(db *database.Database) *MaintenanceManager {
...
func (mm *MaintenanceManager) PreviewBearingReplacement(ctx context.Context, params ReplaceBearingParams) (*models.MaintenanceEffectPreview, error) {
	bearing, err := mm.db.GetBearingByID(ctx, params.BearingID)
	if err != nil {
		return nil, fmt.Errorf("获取轴承信息失败: %w", err)
	}

	newMat, ok := mm.materialsCfg.GetMaterial(params.NewMaterialCode)
	if !ok {
		return nil, errors.New("无效的新材料代码")
	}

	currentStatus, err := mm.db.GetBearingLatestStatusByID(ctx, params.BearingID)
	if err != nil {
	...
	compareResult := mm.engine.CompareMaterials(bearing, materialCodes, loadN, speedRPM, tempC, 8760.0)
	...
	costHint := mm.generateReplacementCostHint(newMat.Code)
	...
	return &models.MaintenanceEffectPreview{
		...
		ActionSummary:      fmt.Sprintf("更换为 %s，预计磨损率降低 %.1f%%", newMat.NameCN, mm.reductionPct(oldWearRate, newWearRate)),
		MaintenanceCostHint: &costHint,
	}, nil
}

func (mm *MaintenanceManager) ExecuteBearingReplacement(ctx context.Context, params ReplaceBearingParams) (*models.MaintenanceRecord, error) {
	preview, err := mm.PreviewBearingReplacement(ctx, params)
	if err != nil {
		return nil, err
	}

	newMat, _ := mm.materialsCfg.GetMaterial(params.NewMaterialCode)
	...
	oldMaterial := mm.guessMaterialFromBearing(bearing)
	...
	err = mm.saveMaintenanceRecord(ctx, rec)
	if err != nil {
	...
	_ = mm.db.UpdateBearingMaterialAndHardness(ctx, params.BearingID, newMat.NameCN, newMat.HardnessHVNominal)

	return rec, nil
}

func (mm *MaintenanceManager) PreviewLubricantAddition(ctx context.Context, params AddLubricantParams) (*models.MaintenanceEffectPreview, error) {
	bearing, err := mm.db.GetBearingByID(ctx, params.BearingID)
	if err != nil {
		return nil, fmt.Errorf("获取轴承信息失败: %w", err)
	}

	lub, ok := mm.lubricantsCfg.GetLubricant(params.LubricantCode)
	if !ok {
	...
	compareResult := mm.engine.CompareLubricants(bearing, materialCode, lubCodes, loadN, speedRPM, tempC, 8760.0)
	...
	costHint := mm.generateLubricantCostHint(lub.Code, params.AmountML)
...
func (mm *MaintenanceManager) ExecuteLubricantAddition(ctx context.Context, params AddLubricantParams) (*models.MaintenanceRecord, error) {
	preview, err := mm.PreviewLubricantAddition(ctx, params)
	if err != nil {
		return nil, err
	}

	lub, _ := mm.lubricantsCfg.GetLubricant(params.LubricantCode)
	...
	rec.WearAfterUm = &wearAfter
	_ = mm.db.UpsertBearingLubricationStatus(ctx, params.BearingID, params.LubricantCode, params.AmountML, params.OperatorName)

	return rec, nil
}

func (mm *MaintenanceManager) GetMaintenanceHistory(ctx context.Context, bearingID, limit int) ([]models.MaintenanceRecord, error) {
	return mm.db.GetMaintenanceRecords(ctx, bearingID, limit)
}

func (mm *MaintenanceManager) GenerateMaintenancePlan(ctx context.Context, bearingID int) (map[string]interface{}, error) {
	...
	if wearPct > 0.9 {
		actions = append(actions, map[string]interface{}{
			...
			"materials": mm.suggestedReplacementMaterials(materialCode, true),
		})
	} else if wearPct > 0.7 {
		actions = append(actions, map[string]interface{}{
			"priority":  "high",
			...
			"materials": mm.suggestedReplacementMaterials(materialCode, false),
		})
	}

	lubFreq, hasFreq := mm.lubricantsCfg.RecommendedLubricationFreq[materialCode]
	if !hasFreq {
		lubFreq = 168
	}
	actions = append(actions, map[string]interface{}{
		...
		"recommended_lubricants": mm.suggestedLubricants(materialCode),
...
func (mm *MaintenanceManager) saveMaintenanceRecord(ctx context.Context, rec *models.MaintenanceRecord) error {
	id, err := mm.db.InsertMaintenanceRecord(ctx, rec)
	if err != nil {
		return err
	}
	rec.ID = id
...
func (mm *MaintenanceManager) guessMaterialCode(bearing *models.Bearing) string {
...
func (mm *MaintenanceManager) guessMaterialFromBearing(bearing *models.Bearing) string {
	code := mm.guessMaterialCode(bearing)
	if mat, ok := mm.materialsCfg.GetMaterial(code); ok {
		return mat.NameCN
	}
	return bearing.Material
}

func (mm *MaintenanceManager) suggestedReplacementMaterials(currentCode string, urgent bool) []map[string]interface{} {
...
func (mm *MaintenanceManager) suggestedLubricants(materialCode string) []map[string]interface{} {
	results := make([]map[string]interface{}, 0)

	mat, isMat := mm.materialsCfg.GetMaterial(materialCode)
	era := "ancient"
	...
	for _, code := range priorityCodes {
		if lub, ok := mm.lubricantsCfg.GetLubricant(code); ok {
			results = append(results, map[string]interface{}{
				"code":            lub.Code,
				"name":            lub.NameCN,
...
func (mm *MaintenanceManager) generateReplacementCostHint(materialCode string) string {
...
func (mm *MaintenanceManager) generateLubricantCostHint(lubricantCode string, amountML float64) string {
	...
	if lub, ok := mm.lubricantsCfg.GetLubricant(lubricantCode); ok {
		lubName = lub.NameCN
	}
	return fmt.Sprintf("%s %.0fml 约 %.1f 元 (%.0f 元/升)", lubName, amountML, totalCost, unitPrice)
}

func (mm *MaintenanceManager) reductionPct(oldVal, newVal float64) float64 {
	if oldVal <= 0 {
		return 0
	}
